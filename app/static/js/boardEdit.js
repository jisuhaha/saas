async function getS3Image(img_uid){
    let imgArr=img_uid.split("|");
    AWS.config.update({
    ...
    for(let i=0; i<imgArr.length; i=i+1){
        let params = { Bucket: 'your_S3_bucket', Key: imgArr[i]};
        let response = await s3.getObject(params).promise();
        let file = new File([response.Body], imgArr[i]);
        dataTranster.items.add(file);
    }
    targetFile.files = dataTranster.files;
    setThumbnail();
}
const completechk = () => {
...
let btnUpload = sec9.querySelector('.btn-upload');
